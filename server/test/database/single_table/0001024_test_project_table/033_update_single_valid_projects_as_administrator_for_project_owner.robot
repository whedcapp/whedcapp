*** Settings ***
Documentation     This test suite tests updating of single valid projects in valid and invalid ways as administrator for project owner. The test case data has been generated by pict using triplet-coverage.
...
${PROJ_KEY3}    'cesar'
${PROJ_KEY4}    'david'
${OWNER}        'testprojectowner@somewhere.org'
...
Update Some Combination
    ...
    ${uidOwner}  Query    SELECT `id_uid` FROM `whedcapp`.`uid` WHERE `uid_text` = ${OWNER}    True
    ...
    @{result3}   Query    SELECT `whedcapp`.`acl_insert_writeOther`(${uidAdmin[0][0]},'${TIME}',${uidOwner[0][0]},${id_acl_level[0][0]},${result1[0][0]})    True
    Log Many    @{result1}
    Run Keyword If    ${success}  
...                   Execute SQL String    CALL `whedcapp`.`project_update_writeOther`(${uidAdmin[0][0]},'${TIME}',${result1[0][0]},${uidOwner[0][0]},'${nstart_date}','${nend_date}',${nproj_key},${nproj_marked_for_deletion})    True
...
Initialize Test Suite
    ...
    ${uidAdmin}  Query    SELECT `id_uid` FROM `whedcapp`.`uid` WHERE `uid_text` = ${ADMIN}    True
    ${uidOwner}    Query    SELECT `whedcapp`.`uid_insert_writeSelf`(${uidAdmin[0][0]},'${TIME}',${OWNER},FALSE,FALSE,FALSE,FALSE)    True
    Row Count Is Equal To X    SELECT * FROM `whedcapp`.`uid` WHERE `id_uid` = ${uidOwner[0][0]} AND NOT `uid_is_whedcapp_administrator`    1     True
